...
def ls(all: bool):
    ...
    console.print(t.error(f"{TRANS('overdue_tasks')}: {len(over_due_list)}"))
    console.print(t.todo(f"{TRANS('todo_tasks')}: {len(todo_list)}"))
    console.print(t.done(f"{TRANS('done_tasks')}: {len(todo_list)}"))
